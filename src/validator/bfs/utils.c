...
bool visit_borders(t_bfs *data)
{
    ...
    {
        if (ft_strchr("0NSEW", data->map.raw[0][i]) || ft_strchr("0NSEW", data->map.raw[data->map.height - 1][i]))
            return false;
        if (data->map.raw[0][i] != '1' && !data->visited[0][i] && !visit_point(i, 0, data))
            return false;
        if (data->map.raw[data->map.height - 1][i] != '1' && !data->visited[data->map.height - 1][i]  && !visit_point(i, data->map.height - 1, data))
            return false;
        i++;
    }
    i = 0;
    ...
    {
        if (ft_strchr("0NSEW", data->map.raw[i][0]) || ft_strchr("0NSEW", data->map.raw[i][data->map.width - 1]))
            return false;
        if (data->map.raw[i][0] != '1' && !data->visited[i][0] && !visit_point(0, i, data))
            return false;
        if (data->map.raw[i][data->map.width - 1] != '1' && !data->visited[i][data->map.width - 1] && !visit_point(data->map.width - 1, i, data))
            return false;
        i++;
    }
    return true;
...
bool visit_neighbourgs(t_point point, t_bfs *data)
{
    ...
    while (i < 4)
    {
        ...
        if (x > 0 && x < data->map.width && y > 0 && y < data->map.height
            && !data->visited[y][x] && data->map.raw[y][x] != '1'
